...
@section('content')
    ...
    <div class="panel panel-flat tb_padd">
        ...
        <a class="btn btn-primary" href="{{ route('user_create') }}"> {{ trans('dash.add_new_user') }} </a>
        ...
        <table class="table table-condensed table-hover datatable-highlight">
            ...
            <tr>
                ...
                <th class="text-center"> {{ trans('dash.identitity_number') }} </th>
            ...
            @forelse($users as $user)
                <tr id="row_{{ $user->id }}">
                    ...
                    <td> {{ $user->mobile }}      </td>
                    <td> {{$user->type }}     </td>
                    <td> {{$user->identitity_number }}        </td>
                    <td> {{$user->full_name }}     </td>

                    <td> {{$user->created_at->format('Y-m-d H:i') }} </td>
                    <td class="text-center">
                        <a href="{{ route('user_edit',['id' => $user->id ]) }}" class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                        <a onclick="sweet_delete( ' {{ route('user_delete').'/'.$user->id }} ' , '{{ trans('dash.deleted_msg_confirm') }}' ,{{ $user->id }} )" class="btn btn-danger" > <i class="icon-database-remove"></i> </a>
                    </td>
                </tr>
