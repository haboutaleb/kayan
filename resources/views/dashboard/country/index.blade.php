...
@section('content')
    ...
    <div class="panel panel-flat tb_padd">
        ...
        <a class="btn btn-primary" href="{{ route('country_create') }}"> {{ trans('dash.add_new_country') }} </a>
        ...
        <table class="table table-condensed table-hover datatable-highlight">
            ...
            <tr>
                ...
                <th class="text-center"> {{ trans('dash.show_key') }} </th>
                <th class="text-center"> {{ trans('dash.show_tel') }} </th>
                <th class="text-center"> {{ trans('dash.continent') }} </th>
            ...
            @forelse($countries as $country)
                <tr id="row_{{ $country->id }}">
                    ...
                    <td> {{$country->show_key }}  </td>
                    <td> {{$country->tel_key }} </td>
                    <td> {{$country->continent }} </td>
                    <td> {{ $country->created_at->format('Y-m-d H:i') }} </td>
                    <td class="text-center">
                        <a href="{{ route('country_edit',['id' => $country->id ]) }}" class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                        <a onclick="sweet_delete( ' {{ route('country_delete').'/'.$country->id }} ' , '{{ trans('dash.deleted_msg_confirm') }}' ,{{ $country->id }} )" class="btn btn-danger" > <i class="icon-database-remove"></i> </a>
                    </td>
                </tr>
