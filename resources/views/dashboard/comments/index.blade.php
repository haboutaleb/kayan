...
@section('content')
    ...
    <div class="panel panel-flat tb_padd">
        ...
        <a class="btn btn-primary" href="{{ route('comment_create') }}"> {{ trans('dash.add_new_comment') }} </a>

        <table class="table table-condensed table-hover datatable-highlight">
            <thead>
            <tr>
                <th class="text-center"> {{ trans('dash.comment-form') }} </th>
                <th class="text-center"> {{ trans('dash.comment-to') }} </th>
                <th class="text-center"> {{ trans('dash.comment') }} </th>
            ...
            @forelse($comments as $comment)
                <tr id="row_{{ $comment->id }}">
                    ...
                    <td class="text-center">
                        <a href="{{ route('comment_edit',['id' => $comment->id ]) }}" class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                        <a onclick="sweet_delete( ' {{ route('comment_delete').'/'.$comment->id }} ' , '{{ trans('dash.deleted_msg_confirm') }}' ,{{ $comment->id }} )" class="btn btn-danger" > <i class="icon-database-remove"></i> </a>
                    </td>
                </tr>
    ...
    <script>
        function sweet_delete($url,$comment,$comment_id)
        {
            $( "#row_"+$comment_id ).css('background-color','#000000').css('color','white');
            swal({
                title: $comment,
                icon: "warning",
                buttons: true,
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $( "#row_"+$comment_id).hide(1000);
                    }else{
                        $( "#row_"+$comment_id ).removeAttr('style');
                    }
                });
        }
    </script>
